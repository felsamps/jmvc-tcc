                           ...
                           Double        dlambda )
{
  ...
  RNOK( m_pcControlMng->initSliceForCoding( rcSliceHeader ) );
	...
	for( uiPos = 0; uiPos < rcList1.getActive(); uiPos++ )
        ...
        #ifdef SLICE_ENCODER_DBG
        fprintf(MemTracingFile::otherFile,"SliceEncoder:: VIEW %u, FRAME %u\n",pcFrame->getViewId(), pcFrame->getPOC());

        //Printing List 0 Contents
        (rcList0.getSize() != 0) ? fprintf(MemTracingFile::otherFile,"SliceEncoder:: LIST 0 CONTENTS (%u) -> ",rcList0.getSize()) : 0;
        for(int i=1;i<=rcList0.getSize();i++) {
            fprintf(MemTracingFile::otherFile,"(%u,%u) ",rcList0[i]->getViewId(),rcList0[i]->getPOC());
        }
        (rcList0.getSize() != 0) ? fprintf(MemTracingFile::otherFile,"\n") : 0;
        

        //Printing List 1 Contents
        (rcList1.getSize() != 0) ? fprintf(MemTracingFile::otherFile,"SliceEncoder:: LIST 1 CONTENTS (%u) -> ", rcList1.getSize()) : 0;
        for(int i=1;i<=rcList1.getSize();i++) {
            fprintf(MemTracingFile::otherFile,"(%u,%u) ",rcList1[i]->getViewId(),rcList1[i]->getPOC());
        }
        (rcList1.getSize() != 0) ? fprintf(MemTracingFile::otherFile,"\n") : 0;

        #endif //SLICE_ENCODER_DBG

        #ifdef TRACING_MODE
            MemTracingFile::setCurrFrame(pcFrame->getPOC());
            MemTracingFile::setCurrView(pcFrame->getViewId());
        #endif
        /**
         * @end
         *
