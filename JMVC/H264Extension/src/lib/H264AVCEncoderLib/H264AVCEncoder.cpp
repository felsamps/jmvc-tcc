...
H264AVCEncoder::init(
    ...
    //Modified by Felipe
    string fileName("view_"), extension(".txt"),folder("mem_tracing/");
    UInt view = pcCodingParameter->getCurentViewId();
    char viewStr[2];
    sprintf(viewStr,"%u",view);
    fileName = folder + fileName + viewStr + extension;

    MemTracingFile::initTraceFile(fileName);

    string otherFileName("debug_");
        ...
        for (UInt uiCurrTempLevel = 0; uiCurrTempLevel < uiTotalTempLevel; uiCurrTempLevel++) {
            for (UInt uiCurrFGSLevel = 0; uiCurrFGSLevel < uiTotalFGSLevel; uiCurrFGSLevel++) {
                if (uiCurrTempLevel >= uiMinTempLevel) {
                    ...
                    if (uiNumScalableLayer == 0) {//JVT-S036
                                ...
                                if (uiCurrLayer) // D != 0, T != 0, Q = 0
                                {
                                    ...
                                    if (uiBaseLayerId == 0 && m_pcCodingParameter->getBaseLayerMode() == 1) // AVC-COMPATIBLE
                                    {
                                        ...
                                        if (uiCurrTempLevel - uiMinTempLevel >= uiBaseTempLevel) {
                                            ...
                                            if (MSYS_UINT_MAX != getScalableLayerId(uiBaseLayerId, uiBaseTempLevel, uiBaseQualityLevel)) { //this should always be true
                                ...
                                if (rcLayer.getLayerCGSSNR()) // D != 0, T != 0, Q = 0
                                {
                                    ...
                                    if (uiBaseLayerCGSSNR == 0 && m_pcCodingParameter->getBaseLayerMode() == 1) // AVC-COMPATIBLE
                                    {
                                        ...
                                        if (uiCurrTempLevel - uiMinTempLevel >= uiBaseTempLevel) {
                                            ...
                                            if (MSYS_UINT_MAX != getScalableLayerId(uiBaseLayerCGSSNR, uiBaseTempLevel, uiBaseQualityLevel)) { //this should always be true
...
H264AVCEncoder::writeParameterSets(ExtBinDataAccessor* pcExtBinDataAccessor, Bool &rbMoreSets) {
    ...
    if (m_bWrteROISEI) {
        LayerParameters& rcLayer = m_pcCodingParameter->getLayerParameters(m_loop_roi_sei / 2);
        {
            ...
            if (m_loop_roi_sei % 2) {
                RNOK(xWriteMotionSEI(pcExtBinDataAccessor, rcLayer.getSGID()[0]));
                m_loop_roi_sei++;
                return Err::m_nOK;
            } else {
                RNOK(xWriteSubPicSEI(pcExtBinDataAccessor, rcLayer.getSLID()[0]));
                m_loop_roi_sei++;
                return Err::m_nOK;
            }
        }
